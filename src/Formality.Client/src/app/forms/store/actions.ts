...
export const getForm = (id: number) => ({ type: getType(getForm), id });
...
export const searchForm = (query?: Api.SearchFormQuery) => ({
  type: getType(searchForm),
  query,
});

export const createForm = (name: string) => ({
  type: getType(createForm),
  name,
});
